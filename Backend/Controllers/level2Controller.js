...
const getAllCases = (req, res) => {
    try {
        ...
        const query = `
            ...
            FROM crime_scene_report_level2 cr
            INNER JOIN check_table2 ct ON cr.case_id = ct.case_id
            GROUP BY cr.case_id;
        `;
...
const checkAnswer = (req, res) => {
    ...
    try {
        ...
        pool.query(`SELECT * FROM users WHERE id = ? LIMIT 1`, [userId], (error, userResults) => {
            ...
            pool.query(
                `SELECT completed FROM user_progress WHERE user_id = ? AND case_id = ? AND level_id = 2 LIMIT 1`,
                ...
                (progressError, progressResults) => {
                    ...
                    pool.query(
                        ...
                        [numericCaseId, ...personIds], (error, answerResults) => {
                            ...
                            pool.query(
                                `SELECT COUNT(*) as totalPersons FROM check_table2 WHERE case_id = ?`,
                                [numericCaseId],
                                (error2, totalResults) => {
                                    ...
                                    if (uniqueFoundPersons === personIds.length && foundPersons === totalPersons) {
                                        ...
                                        if (alreadyCompleted) {
                                            ...
                                            pool.query(`
                                                ...
                                                WHERE user_id = ? AND case_id = ? AND level_id = 2
